...
const moment = require('moment'); // <<< adiciona o moment para formatDate
...
// Configuração do Handlebars com helper formatDate
app.engine('handlebars', exphbs.engine({
  ...
  helpers: {
    formatDate: (date) => {
      if (!date) return '';
      return moment(date).format('DD/MM/YYYY HH:mm'); // Formato BR
    }
  }
}));
...
app.post('/address/create', async (req, res) => {
  try {
    ...
    if (!street || street.trim().length < 5) return res.redirect(`/users/edit/${userId}`);
    if (!city || city.trim().length < 2) return res.redirect(`/users/edit/${userId}`);
    const addressData = {
      street: street.trim(),
...
app.post('/address/delete', async (req, res) => {
  try {
    ...
    if (deletedRows > 0) console.log(`Endereço ${id} excluído`);
